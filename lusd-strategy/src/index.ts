...
export function checker(args: Args_checker): CheckerResult {
  ...
  const interval = userArgs.interval;
  const strategy = userArgs.strategy;
  const rewardSwappingSlippagePercent = userArgs.rewardSwappingSlippagePercent;
  const maxBentoBoxAmountIncreasePercent =
    userArgs.maxBentoBoxAmountIncreasePercent;
  const maxBentoBoxChangeAmountPercent =
    userArgs.maxBentoBoxChangeAmountPercent;
